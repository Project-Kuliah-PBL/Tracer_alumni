...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .page-subtitle { font-size:1rem; color:var(--color-text); max-width: 720px; }

        /* ═══ CONTENT GRID ═══ */
        .content-grid { display: grid; grid-template-columns: 1fr 280px; gap: 1.25rem; align-items: start; }
        
        .card, .tips-card { min-width: 0; }
        ...
        .card-header { padding: 1.25rem 1.5rem 1rem; border-bottom: 1px solid var(--color-border); }
        .card-header h2 { font-family: var(--font-heading); font-size: 1rem; font-weight: 700; color: var(--color-secondary); margin-bottom: .2rem; }
        .card-header p { font-size: .8125rem; color: var(--color-muted); }
        .card-body { padding: 1.5rem; display: flex; flex-direction: column; gap: 1.125rem; }

        /* ═══ FORM ═══ */
        .form-group { display: flex; flex-direction: column; gap: .4rem; }
        .form-label { font-size: .8125rem; font-weight: 600; color: var(--color-secondary); }
        .input-wrapper { position: relative; display: flex; align-items: center; }
        .input-icon { position: absolute; left: .875rem; display: flex; align-items: center; pointer-events: none; }
        .input-icon svg { width: 1rem; height: 1rem; stroke: #b0b8c9; stroke-width: 1.75; fill: none; }

        .form-input {
            width: 100%; padding: .7rem 2.75rem .7rem 2.75rem;
            border: 1.5px solid var(--color-border); border-radius: var(--radius-sm);
            font-family: var(--font-body); font-size: .9rem; color: var(--color-text);
            background: var(--color-surface); transition: border-color .2s, box-shadow .2s; outline: none;
        }
        .form-input::placeholder { color: #b8bfcc; font-size: .875rem; }
        .form-input:focus { border-color: var(--color-primary); box-shadow: 0 0 0 3px rgba(0, 63, 135, .08); }
        .form-input.is-error { border-color: var(--color-danger); box-shadow: 0 0 0 3px rgba(209, 41, 36, .1); }
        .form-input.is-error:focus { border-color: var(--color-danger); box-shadow: 0 0 0 3px rgba(209, 41, 36, .15); }

        .toggle-password { position: absolute; right: .875rem; background: none; border: none; cursor: pointer; padding: 0; display: flex; align-items: center; color: #b0b8c9; transition: color .2s; }
        ...
        /* ═══ STRENGTH BAR ═══ */
        .strength-wrapper { display: none; align-items: center; gap: .5rem; margin-top: .3rem; }
        .strength-bar-track { flex: 1; height: 4px; background: var(--color-border); border-radius: var(--radius-full); overflow: hidden; }
        .strength-bar-fill { height: 100%; border-radius: var(--radius-full); width: 0%; background: #f59e0b; transition: width .4s ease, background .4s ease; }
        .strength-label { font-size: .72rem; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; color: #f59e0b; white-space: nowrap; min-width: 72px; text-align: right; }

        /* ═══ FORM ACTIONS ═══ */
        .form-actions { display: flex; justify-content: flex-end; gap: .75rem; padding-top: .75rem; border-top: 1px solid var(--color-border); margin-top: .25rem; }
        .btn-cancel { padding: .625rem 1.25rem; border: 1.5px solid var(--color-border); border-radius: var(--radius-sm); background: transparent; font-family: var(--font-body); font-size: .875rem; font-weight: 600; color: var(--color-text); cursor: pointer; transition: border-color .2s, background .2s; }
        .btn-cancel:hover { background: var(--color-bg); border-color: #c5cad6; }
        .btn-save { padding: .625rem 1.375rem; border: none; border-radius: var(--radius-sm); background: var(--color-primary-btn); font-family: var(--font-body); font-size: .875rem; font-weight: 600; color: #fff; cursor: pointer; transition: background .2s; }
        .btn-save:hover { background: #004f87; }

        /* ═══ TIPS CARD ═══ */
        .tips-card { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); overflow: hidden; }
        .tips-header { display: flex; align-items: center; gap: .625rem; padding: 1rem 1.125rem; border-bottom: 1px solid var(--color-border); background: var(--color-primary-soft); }
        .tips-icon { width: 2rem; height: 2rem; background: var(--color-primary); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .tips-icon svg { width: 1rem; height: 1rem; stroke: #fff; stroke-width: 1.75; fill: none; }
        .tips-header h3 { font-family: var(--font-heading); font-size: .9375rem; font-weight: 700; color: var(--color-primary); }
        .tips-body { padding: 1rem 1.125rem; display: flex; flex-direction: column; gap: .5rem; }
        .tips-intro { font-size: .8125rem; color: var(--color-primary); background: #dbeafe; border-left: 3px solid var(--color-primary); border-radius: 0 var(--radius-sm) var(--radius-sm) 0; padding: .6rem .75rem; line-height: 1.55; margin-bottom: .25rem; }
        .tip-item { display: flex; align-items: flex-start; gap: .55rem; font-size: .8rem; color: var(--color-text); line-height: 1.5; padding: .15rem 0; }
        .tip-item-icon { width: 1rem; height: 1rem; flex-shrink: 0; margin-top: .15rem; }
        ...
        @media(max-width:1024px){ 
            ...
            .form-input { padding: .7rem 2.5rem .7rem 2.5rem; } 
        }
        @media(max-width:768px){
            .content-grid { gap: 1rem; }
            .card, .tips-card { border-radius: .85rem; }
            .form-actions { justify-content: stretch; }
            .form-actions .btn-cancel, .form-actions .btn-save { flex: 1 1 100%; margin-right: 0; }
            .strength-label { min-width: auto; }
            .tips-header { flex-wrap: wrap; gap: .75rem; }
        }
    </style>
</head>
<body class="bg-[#F1F5F9] h-screen flex flex-col font-sans">
    ...
    <div class="flex flex-1 overflow-hidden w-full relative">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 custom-scroll">
            ...
            @if (session('success'))
                <div id="successAlert" class="mb-4" style="position: relative; padding: 1rem 1.125rem; background: #f0fdf4; border: 1px solid #86efac; border-radius: 0.75rem; display: flex; align-items: flex-start; gap: 0.75rem;">
...
<script>
    ...
    function toggleSidebar() {
        ...
        // Gunakan toggle pada class transform tailwind
        sidebar.classList.toggle('-translate-x-full');
        overlay.classList.toggle('hidden');
        
        // Matikan scroll body jika sidebar terbuka
